...
# OpenForge — Termux / proot-Debian launcher.
#
# Starts the Python bridge (server/) which serves the same web UI and API as
# the native Android Go daemon, so the browser/PWA flow keeps working on
# devices where the APK is not used.
set -e
cd "$(dirname "$0")"
...
# 3. Start the bridge
echo "[*] Bridge starting → open http://127.0.0.1:${OCMB_PORT:-8787}/ui in your browser"
